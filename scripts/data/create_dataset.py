...
def heuristic(row):
    ...
    for src_word, tgt_word in zip(aligned_src, aligned_tgt):
        if aligned_src.count(src_word) > 1 and aligned_tgt.count(tgt_word) < 2:
            result.extend(["｟mrk_fuzzy｠", src_word])
        else:
            result.append(src_word)
    return " ".join(result)

def main(src_file, tgt_file, output_dir):
    """Función principal para la alineación de frases en archivos fuente y objetivo."""
    df = pd.DataFrame({'source': open_file(src_file), 'target': open_file(tgt_file)})
    print("Data loaded!")
    df['new_source'] = df.progress_apply(heuristic, axis=1)
    save_file(df.new_source.to_list(), f'{output_dir}/ja.txt')
    save_file(df.target.to_list(), f'{output_dir}/en.txt')
