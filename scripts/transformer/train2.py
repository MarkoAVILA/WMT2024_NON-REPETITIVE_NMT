...
from transformer.dataset import create_inference_dataset, create_training_dataset
from transformer.evaluate2 import evaluate
...
def main():
    ...
    parser.add_argument("--batch_size", required=False, help="batch size level tokens")
    parser.add_argument("--bpe_tgt", required=False, help="bpe_tgt")
...
def train(
    ...
    batch_size = int(bs) #config.get("batch_size") #int(bs)
    ...
    if config.get("batch_autotune", False):
        ...
        effective_batch_size = 25*int(bs)  #config.get("effective_batch_size") #25*int(bs) 
        accum_steps = (
    ...
    for batches in dataset:
        ...
        if dataset_valid is not None and step % config.get("validation_every") == 0:
            ...
            model.train()
            # logger.info("step = %d" " ; validation loss = %f", step, valid_loss)
