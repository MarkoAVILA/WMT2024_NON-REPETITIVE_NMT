...
class EncodeTokens:
    ...
    def __call__(self, element):
        ...
        if target is None:
            return collections.OrderedDict({"indexes":idx, "source": source})
        ...
        output = collections.OrderedDict(
            {"indexes": idx, "source": source, "target_in": target_in, "target_out": target_out}
        )
        el.update(output)
        el.pop("target", None)
...
class ZipDataset:
    ...
    def __iter__(self):
        for idx, elements in enumerate(itertools.zip_longest(*self._datasets)):
            yield idx, elements
...
class ZipMapDataset:
    ...
    def __iter__(self):
        ...
        for idx, elements in enumerate(zip(*self._datasets.values())):
