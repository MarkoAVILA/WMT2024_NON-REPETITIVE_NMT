...
class CT2Converter(ctranslate2.converters.Converter):
    ...
    def _load(self):
        ...
        set_embeddings(
            spec.encoder.embeddings[0], weights, "src_embeddings.word_embeddings"
        )
        set_embeddings(
            spec.decoder.embeddings, weights, "tgt_embeddings.word_embeddings"
        )

        set_position_encodings(
            spec.encoder.position_encodings,
            weights,
            "src_embeddings.position_embeddings",
        )
        set_position_encodings(
            spec.decoder.position_encodings,
            weights,
            "tgt_embeddings.position_embeddings",
        )

        set_layer_norm(spec.encoder.layer_norm, weights, "encoder.norm")
        set_layer_norm(spec.decoder.layer_norm, weights, "decoder.norm")

        if "output_layer.weight" in weights:
            set_linear(spec.decoder.projection, weights, "output_layer")
        else:
            set_linear(
                spec.decoder.projection, weights, "tgt_embeddings.word_embeddings"
            )

        # Sometimes weights are shared, but bias is not
        if "output_layer.bias" in weights:
            spec.decoder.projection.bias = weights["output_layer.bias"].numpy()
...
def set_position_encodings(spec, weights, scope):
    spec.encodings = weights[scope].numpy()
